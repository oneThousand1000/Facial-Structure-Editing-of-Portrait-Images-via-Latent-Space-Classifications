...
from interface.models.model_settings import MODEL_POOL
from interface.models.pggan_generator import PGGANGenerator
from interface.models.stylegan_generator import StyleGANGenerator
...
def parse_args():
    ...
    parser.add_argument('-o', '--output_dir', type=str, default='data/stylegan_ffhq_add',
...
def main():
    ...
    for latent_codes_batch in model.get_batch_inputs(latent_codes):
        ...
        val=outputs[key]
        for image in val:
            save_path = os.path.join(args.output_dir, f'{count + 50000:06d}.jpg')
            cv2.imwrite(save_path, image[:, :, ::-1])

            score = check_double_chin(img_path=save_path, model=double_chin_checker)
            pbar.update(1)
            if (score == 1):
                # cv2.imwrite(os.path.join('F:/DoubleChin/datasets/CelebAMask-HQ/generated_double_chin_img/temp',
                #                          f'{pbar.n:06d}.jpg'), image[:, :, ::-1])
                choose.append(True)
                scores.append(score)
                count += 1
            else:
        ...
        if pbar.n % 1000 == 0 or pbar.n == total_num:
            logger.debug(f'  Finish {count:6d} samples.')
    pbar.close()
