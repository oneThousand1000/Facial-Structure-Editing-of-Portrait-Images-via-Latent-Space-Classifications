...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Train semantic boundary with given latent codes and '
                    'attribute scores.')
    parser.add_argument('-o', '--output_dir', type=str,default='boundaries/coarse/styleSpace/stylegan2_ada_double_chin',
                        help='Directory to save the output results. (required)')
    parser.add_argument('-c', '--latent_codes_dir', type=str, default='F:/DoubleChin/datasets/ffhq_gen_data/stylegan2_ada_psi_0.8_double_chin_S_space',
                        help='Path to the input latent codes. (required)')
    parser.add_argument('-s', '--scores_path', type=str, default='F:/DoubleChin/datasets/ffhq_gen_data/stylegan2_ada_psi_0.8_double_chin_S_space/double_chin_scores.npy',
                        help='Path to the input attribute scores. (required)')
    parser.add_argument('-n', '--chosen_num_or_ratio', type=float, default=0.099,
...
def main():
    ...
    for i in range(26):
        latent_codes_path=os.path.join(args.latent_codes_dir,f's_{i}.npy')
