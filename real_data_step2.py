...
def parse_args():
    ...
    parser.add_argument('-i', '--data_dir', type=str, default='F:/DoubleChin/datasets/ffhq_data/real_img',
...
def diffuse(args, img_path, mask_path, latent_path,inverter=None):
    ...
    init_code = np.reshape(np.load(latent_path),(1,18,512))
    ...
    latent_code_save_path = os.path.join(os.path.join(args.data_dir, 'code'),
    ...
    origin_img = resize_image(
        load_image(os.path.join(os.path.join(args.data_dir, 'origin'), image_name + '.jpg')),
    ...
    image_viz_save_path = os.path.join(os.path.join(args.data_dir, 'viz'), '%s.jpg' % image_name)
    res = np.concatenate([origin_img, res], axis=1)
    save_image(image_viz_save_path, res)
    #
    # res_path = os.path.join(os.path.join(args.data_dir, 'res'), image_name.replace('_res', '') + '.jpg')
    #
    # save_img =np.concatenate([origin_img,res,target_image],axis=1)
    #
    #
...
def run():
    ...
    mask_list = [path.replace('simple_synthesis', 'mask').replace('jpg', 'png') for path in
                 image_list]
    latent_list = [path.replace('simple_synthesis', 'code').replace('.jpg', '_wp.npy') for path in
                 image_list]
    image_num = len(image_list)

    assert args.model_name == 'stylegan2_ffhq'
    ...
    for index in range(image_num):
        ...
        if os.path.exists( os.path.join(os.path.join(args.data_dir, 'res'), '%s.jpg' % image_name)):
            continue
