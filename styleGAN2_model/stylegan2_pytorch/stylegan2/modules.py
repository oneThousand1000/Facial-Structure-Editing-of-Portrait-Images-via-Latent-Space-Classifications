...
def _setup_filter_kernel(filter_kernel, gain=1, up_factor=1, dim=2):
    ...
    filter_kernel = filter_kernel* gain * up_factor ** 2
...
class NoiseInjectionWrapper(nn.Module):
    ...
    def __init__(self, layer, same_over_batch=True,noise_input=None):
        super(NoiseInjectionWrapper, self).__init__()
        self.layer = layer
        self.weight = torch.nn.Parameter(torch.zeros(1))
        self.register_buffer('noise_storage', noise_input)
        self.same_over_batch = same_over_batch
        self.random_noise()
        # if not self.has_noise_shape():
        #     #print('ramdom!')
        #     self.random_noise()
        # else:
        #     self.static_noise()
    ...
    def forward(self, *args, **kwargs):
        ...
        x =x+ self.weight * self.noise_storage
        return x
...
class GeneratorConvBlock(nn.Module):
    ...
    def forward(self, input, latents=None, **kwargs):
        ...
        if self.projection is not None:
            x =x+ self.projection(input=input)
            x = x* self.res_scale
        return x
