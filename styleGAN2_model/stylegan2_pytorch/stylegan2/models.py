...
class Generator(_BaseModel):
    ...
    def _serialize(self, half=False):
        ...
        return state
    def set_noise(self,noise_input):
    ...
    def set_truncation(self, truncation_psi=None, truncation_cutoff=None):
        ...
        if truncation_psi is not None and truncation_psi != 1 and truncation_cutoff != 0:
            ...
            if truncation_cutoff is None:
                layer_psi = layer_psi*truncation_psi
            else:
                layer_psi_mask = torch.arange(len(layer_psi)) < truncation_cutoff
                layer_psi[layer_psi_mask] = layer_psi[layer_psi_mask]* truncation_psi
    ...
    def truncate(self, dlatents):
        ...
        """
        #print('dlatent_avg.size:',self.dlatent_avg.size())
        if self.layer_psi is not None:
            dlatents = utils.lerp(self.dlatent_avg, dlatents, self.layer_psi)
        return dlatents

    def run_mapping(self,latents,labels=None,mapping_grad=True):
        ...
        return dlatents
    def run_truncation(self,dlatents):

        dlatents = dlatents.view(-1, self.num_latents, dlatents.size(-1))
        if self.num_latents == 1:
            dlatents = dlatents.expand(
                dlatents.size(0), len(self), dlatents.size(2))  # N,512 -> N,18,512
        elif self.num_latents != len(self):
    ...
    def forward(self,
        ...
        if dlatents is None:
            ...
            if  len(latents.shape) == 3:
                num_latents = latents.size(1) #1
                latents = latents.view(-1, latents.size(-1)) # 1,512
                # Labels need to repeated for the extra dimension of latents.
                if labels is not None:
